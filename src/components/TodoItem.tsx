...
export default function TodoItem({task, section, onComplete, onEdit, onPriorityChange, onTextInputBlur, onDelete}: TodoItemProps) {
  ...
  return (
    <li
    key={task.id}
    className="relative flex gap-2 items-center mb-3 justify-between hover:bg-background px-1"
  >
    <input
      name="todoCheckbox"
      type="checkbox"
      ...
      className={`${task.priority} appearance-none flex-shrink-0 cursor-pointer h-5 w-5 border-2 rounded-md focus:outline-none checkbox`}
    />
    <input
      name="todoText"
      type="text"
