  ...
  return (
    <div ref={dropDownRef} className="absolute right-0 -top-24 z-10 p-2 bg-primary border-2 border-accent border-opacity-50 rounded-xl">
      ...
      {priorities.map((newPriority) => {
        return <button key={newPriority} aria-label={`${newPriority} Priority"`} onClick={() => onPriorityChange(sectionId, todoItem, newPriority)} className={`${newPriority === "high" ? "text-high" : newPriority === "medium" ? "text-medium" : newPriority === "low" ? "text-low" : "text-accent"} p-0.5 hover:bg-slate-200 hover:bg-opacity-50 rounded`}>
